...
export async function GetReviewsForGame(gameName) {
  ...
  const location = "/users/{user_id}/reviews/{game}".replace(
    "{game}",
    gameName
  );
...
export async function GetTeammatesForGame(gameName) {
  // Set the location to the game name
  const location = "/users/{user_id}/teammates/{game}".replace(
    "{game}",
    gameName
  );
...
function convertGameReviewsData(gameReviewsResponse) {
  ...
  for (let i = 0; i < gameReviewsResponse.length; i++) {
    ...
    const data = {
      ...
      images: (gameReviewsResponse[i].ImageURL || []).map((url) => {
        return { url: url };
      }),
